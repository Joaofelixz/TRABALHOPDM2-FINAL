window.addEventListener("DOMContentLoaded", ()=>{
  ...
  request.onupgradeneeded = e =>{
    db = e.target.result;
    db.createObjectStore("locais", {keyPatch: "id", autoIncrement: true})
  };
  request.onsuccess = e =>{
    db= e.target.result;
    listarLocais();
  ...
  function listarLocais(){
    ...
    store.getAll().onsuccess = e =>{
      ...
      ? locais.map(l => `
        <div>
        <p><b>${l.nome}</b></p>
        <button onclick="verMapa('${encodeURIComponent(l.nome)}')">Ver Mapa</button>
        <button onclick="removerLocal(${l.id})">Remover</button>
        </div>`).join("") 
